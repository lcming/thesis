    ...
    \begin{frame}{Contribution}
        \begin{itemize}
            ...
            \begin{frame}{Basic Design Considerations}
                ...
                \end{itemize}
                \large{That's "D"ual-thr"e"ad "Ar"chitecture --- DeAr}
            \end{frame}
            ...
            \begin{frame}{Advanced Design Considerations}
                ...
                \centering
                {
                    \animategraphics[loop,controls,width=0.8\linewidth]{12}{./gif/tree-}{0}{10}
                }
            \end{frame}
            ...
            \begin{frame}{DeAr Micro-architecture with SBRF}
                \begin{columns}
                    ...
                    \begin{column}{0.35\textwidth}
                        \begin{block}<2->{SBRF}
                            \begin{itemize}
                                \item Two load-queues
                                \item Two store-queues
                                \item Two i.m.-stacks
                            \end{itemize}    
                        \end{block}
                    \end{column}
            ...
            \begin{frame}{Instruction Set Architecture (1/2)}
                ...
                \begin{itemize}
                    ...
                    \item <3->{RISC-fashion portion: a subset of RISC ISA}
                \end{itemize}
                \begin{table}[ht!]
                    \centering
            ...
            \begin{frame}{Integrating DeAr with HSA (2/3)}
                \begin{itemize}
                    \item <2-> {Multi-core DeAr with shared L1 I-cache~\cite{kelly2004shared} --- fitting HSA SPMD and eliminating program duplication}
                ...
                \begin{enumerate}
                    \item <2->{Compilation (front-end)
                            \begin{itemize}
                                ...
                                \item Off-line compiler~\cite{cloc} compiles them to HSAIL
                            \end{itemize}
                        }
                    ...
                    \begin{column}{0.55\textwidth}
                        \begin{enumerate}
                            ...
                            \item <3->{SSA renames variables.
                                \begin{itemize}
                                    \item Assign new name to each LHS variable
            ...
            \begin{frame}{HDFG-based scheduling: Inter-tree parallelism}
                ...
                \begin{itemize}
                    \item <6->{Only one $BT$ exists in HDFG}
            ...
            \begin{frame}{HDFG-based scheduling: Intra-tree parallelism}
                \begin{columns}
                    ...
                    \begin{column}{0.65\textwidth}
                        \begin{itemize}
                            \item <2->{Partitioning $BT$ into three parts --- root, left and right-subtrees
                                \begin{itemize}
                                    ...
                                    \item Left and right-subtrees --- \textbf{intra-tree parallelism}, processing concurrently
                                \end{itemize}
                            }
                        \item <3->{Imbalance subtree lead to partial $BT$ remaining after scheduling}
                        ...
                        \vspace{1em} 
                        \centering
                        \only<6->{\textbf{Dynamic programming (DP) based ALU-allocation!}}
                    \end{column}
                \end{columns}
            \end{frame}
            ...
            \begin{frame}{Operations per Cycle}
                ...
                \begin{itemize}
                    ...
                    \item <4->{DeAr have only 1.11\% and 4.26\% loss against VLIW, 
