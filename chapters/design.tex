...
its wire area and power dissipation are significantly improved.%, as discussed in Section~\ref{sec:rfcell}.
...
while a \textit{POP} eliminate the head data by subtracting the head address by 1.
Since a \textit{POP}followed by a \textit{PUSH} modifies the data at the head data and maintains the head address, 
a \textit{MODIFY} is defined to simplify the usage of two operations.
...
%----------------------------------------------
\indent
To introduce the collaboration mechanism of threads, We further present \textproc{Inter-tree scheduling} and \textproc{Intra-tree scheduling}, 
both of which require Algorithm~\ref{alg:sbt}.
A typical HDFG contains multiple $bt$.
The DeAr scheduler performs \textproc{Inter-tree scheduling} if multiple $bt$s exist.
Algorithm~\ref{alg:inter} illustrates the detail of \textproc{Inter-tree scheduling}, 
...
After above steps, two work-queues are filled, and the key part of the algorithm, \textproc{ALU Alloc}, is performed.
\textproc{ALU Alloc} dispatches operations from two work-queues which can execute concurrently, 
...
After \textproc{ALU Alloc}, there will be at least one work-queue is consumed, 
...
Remaining operations are thus reverted to the form of $bt$ by the routine \textproc{Restore Subtree from Queue},
...
%------------------------------------------
\indent 
To deal with the $bt_{remain}$, \textproc{Intra-tree Scheduling}, illustrated in Algorithm~\ref{alg:intra}, is applied.
